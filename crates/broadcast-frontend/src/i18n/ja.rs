...
pub static STRINGS: Strings = Strings {
    ...
    prepare_tutorials: "解説者によるチュートリアルへのリンクは、用意でき次第ここに追加\
        します。お使いのウォレットでのやり方がわからない場合は、あとでこのページを\
        見直してもかまいません。",
    ...
    faq_a_where: "デコード、ファイナライズ、手数料の計算はすべてブラウザ内で行われ、\
        このサイトは間に一切サーバーを置いていません。このページ（およびあなたの\
        コンピュータ）から出ていくのは、送信を押したときにブラウザから MARA Slipstream \
        へ直接送られる、ファイナライズ済みトランザクションの hex だけです。",
